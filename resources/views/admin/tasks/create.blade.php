      ...
      <main>
         ...
         <div class="w-full mx-auto p-6 sm:p-8">
            <div class="flex flex-col mb-4 border-b border-gray-300 pb-4">
                <h1 class="block font-semibold leading-6 text-xl mb-1">Tambah Tugas Baru</h1>
             </div>
             <form action="{{ route('admin.tasks.store') }}" method="POST" class="space-y-6 bg-white p-6 rounded shadow-md">
                @csrf
            
                {{-- Nama Tugas --}}
                <div>
                    <label class="block font-medium mb-1">Nama Tugas:</label>
                    <input type="text" name="nama_task" value="{{ old('nama_task') }}" required
                ...
                <div>
                    ...
                    <input type="date" name="waktu_pengerjaan" value="{{ old('waktu_pengerjaan') }}" required
                        class="w-full border border-gray-300 rounded p-2 focus:ring focus:ring-indigo-300">
                </div>
                ...
                <div>
                    ...
                    <select name="owner_id" required
                        ...
                        @foreach ($customers as $customer)
                            <option value="{{ $customer->customer_id }}" {{ old('owner_id') == $customer->customer_id ? 'selected' : '' }}>
                                {{ $customer->nama }}
                            </option>
                        @endforeach
                    </select>
